...
| 2026-02-28 | `engine/src/graphics/Shader.cs` | Added `Shader.CreateRaw()` internal factory method | Phase 1b: Create Shader objects from WGSL source without binary asset pipeline |
| 2026-02-28 | `platform/webgpu/WebGPUGraphicsDriver.cs` | Added `CreateDepthTexture()` call at end of `CreateSwapChain()` | Bug fix: depth texture was never created during init, causing WebGPU validation error on first frame |

### Phase 1a Note

Phase 1a (depth pipeline) was found to already be complete in the upstream fork. ShaderFlags.Depth/DepthLess are wired through ShaderInfo → PsoKey → CreateRenderPipeline(). Depth texture is created and attached in BeginScenePass/ResumeScenePass. No fork changes were needed for 1a.
